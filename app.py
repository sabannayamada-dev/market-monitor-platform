...
APP_VERSION = "5.7.1"
LOCAL_DATABASE_PATH = APP_DIRECTORY / "stock_cache.db"
LEGACY_DATABASE_PATH = Path.home() / "Documents" / "Codex" / "2026-06-23" / "python-csv" / "stock_cache.db"


def resolve_database_path() -> Path:
    configured = os.environ.get("STOCK_CACHE_DB", "").strip()
    if configured:
        return Path(configured)
    if LOCAL_DATABASE_PATH.exists() or not LEGACY_DATABASE_PATH.exists():
...
DATAFORSEO_TRENDS_URL = (
    ...
    "dataforseo_trends/explore/live"
)
DATAFORSEO_SECRET_PATH = APP_DIRECTORY / "dataforseo_credentials.dpapi"
...
def initialize_database() -> None:
    with database_connection() as connection:
        connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS dataforseo_trends_cache (
                ...
                fetched_at TEXT NOT NULL,
                PRIMARY KEY (keywords_key, start_date, end_date, location_name)
            );
            """
        )
        columns = {
...
def load_dataforseo_credentials() -> tuple[str, str]:
    ...
    try:
        ...
        stored = load_credentials(DATAFORSEO_SECRET_PATH)
    except Exception:
        stored = {}
    login = os.getenv("DATAFORSEO_LOGIN", "").strip() or stored.get(
        "dataforseo_login", ""
    ).strip()
    password = os.getenv("DATAFORSEO_PASSWORD", "").strip() or stored.get(
        "dataforseo_password", ""
    ).strip()
    return login, password


def save_dataforseo_credentials(login: str, password: str) -> None:
    login = str(login or "").strip()
    password = str(password or "").strip()
    if not login or not password:
        raise ValueError("DataForSEOのloginとpasswordを両方入力してください。")
    from patent_monitor.secure_store import save_credentials
    ...
    save_credentials(
        ...
        DATAFORSEO_SECRET_PATH,
    )


def delete_dataforseo_credentials() -> bool:
    try:
        from patent_monitor.secure_store import delete_credentials

        return delete_credentials(DATAFORSEO_SECRET_PATH)
    except Exception:
        return False


def dataforseo_credential_status() -> dict:
    login, password = load_dataforseo_credentials()
    masked_login = ""
    if login:
        masked_login = login[:3] + "***" + login[-3:] if len(login) > 7 else "設定済み"
    return {
        "configured": bool(login and password),
        "maskedLogin": masked_login,
        "source": "environment"
...
def normalize_trends_keywords(raw_keywords: list[str]) -> list[str]:
    keywords: list[str] = []
    ...
    if not keywords:
        raise ValueError("検索語を1語以上入力してください。")
    if len(keywords) > 3:
        raise ValueError("1タスクに指定できる検索語は3語までです。")
    if any(not 1 < len(keyword) <= 80 for keyword in keywords):
        raise ValueError("各検索語は2〜80文字で入力してください。")
    return keywords


def dataforseo_trends_payload(
    raw_keywords: list[str],
    start_date: str,
    end_date: str,
    location_name: str = "Japan",
    normalization_mode: str = "independent",
) -> dict:
    keywords = normalize_trends_keywords(raw_keywords)
    normalization_mode = str(normalization_mode or "independent").strip().lower()
    if normalization_mode not in {"independent", "shared"}:
        raise ValueError("取得方式はindependentまたはsharedを指定してください。")

    if normalization_mode == "independent":
        responses = [
            dataforseo_trends_payload(
                [keyword],
                start_date,
                end_date,
                location_name,
        ...
        return {
            "provider": "DataForSEO Trends Explore",
            "keywords": keywords,
            "locationName": responses[0]["locationName"],
            "startDate": start_date,
            "endDate": end_date,
            "coverageStartDate": responses[0]["coverageStartDate"],
            "series": [item for response in responses for item in response["series"]],
            "costUsd": sum(
                0 if response["cacheUsed"] else float(response.get("costUsd") or 0)
                for response in responses
            ),
            "cacheUsed": all(response["cacheUsed"] for response in responses),
            "cacheHitCount": sum(bool(response["cacheUsed"]) for response in responses),
            "requestCount": sum(int(response.get("requestCount", 0)) for response in responses),
            "normalizationMode": "independent",
            "note": "各検索語を単独取得した0〜100の相対人気度です。語同士の大小比較には使えません。",
        }
    try:
        start = datetime.strptime(start_date, "%Y-%m-%d")
        end = datetime.strptime(end_date, "%Y-%m-%d")
    except ValueError as exc:
        raise ValueError("DataForSEO Trendsの表示期間を決定できませんでした。") from exc
    if start >= end:
        raise ValueError("DataForSEO Trendsの開始日は終了日より前である必要があります。")
    effective_start_date = max(start, datetime(2004, 1, 1)).strftime("%Y-%m-%d")
    location_name = re.sub(r"[^A-Za-z ]", "", str(location_name or "Japan")).strip() or "Japan"
    keywords_key = json.dumps(keywords, ensure_ascii=False, separators=(",", ":"))

    initialize_database()
    with database_connection() as connection:
        cached = connection.execute(
            """
            SELECT series_json, cost_usd, fetched_at
            FROM dataforseo_trends_cache
            WHERE keywords_key = ? AND start_date = ? AND end_date = ?
              AND location_name = ?
            """,
            (keywords_key, effective_start_date, end_date, location_name),
        ).fetchone()
    if cached:
        fetched_at = datetime.strptime(cached["fetched_at"], "%Y-%m-%d %H:%M:%S")
        if (datetime.utcnow() - fetched_at).total_seconds() <= 24 * 60 * 60:
            return {
                "provider": "DataForSEO Trends Explore",
                "keywords": keywords,
                ...
                "startDate": start_date,
                "endDate": end_date,
                "coverageStartDate": effective_start_date,
                "series": json.loads(cached["series_json"]),
                "costUsd": float(cached["cost_usd"] or 0),
                "cacheUsed": True,
                "cacheHitCount": 1,
                "requestCount": 0,
                "normalizationMode": "shared",
                "note": "DataForSEO独自推定による0〜100の相対人気度です。実検索回数ではありません。",
            }

    login, password = load_dataforseo_credentials()
    if not login or not password:
        raise RuntimeError(
            "DataForSEO認証が未設定です。DataForSEO設定を開き、API Accessのloginとpasswordを保存してください。"
        )
    ...
    request = Request(
        ...
        headers={
            ...
            "User-Agent": f"StockBottomApp/{APP_VERSION}",
        },
        method="POST",
    )
    try:
        ...
        raise RuntimeError(f"DataForSEO HTTP {exc.code}: {details[-1000:]}") from exc
    except (URLError, TimeoutError) as exc:
        raise RuntimeError(f"DataForSEOへ接続できませんでした: {exc}") from exc

    if int(payload.get("status_code", 0)) != 20000:
        raise RuntimeError(
            f"DataForSEOエラー {payload.get('status_code')}: {payload.get('status_message', '')}"
        )
    tasks = payload.get("tasks") or []
    if not tasks:
        raise RuntimeError("DataForSEOからタスク結果が返されませんでした。")
    task = tasks[0]
    if int(task.get("status_code", 0)) != 20000:
        raise RuntimeError(
            f"DataForSEOタスクエラー {task.get('status_code')}: {task.get('status_message', '')}"
        )
    results = task.get("result") or []
    items = (results[0].get("items") or []) if results else []
    graph = next(
        (item for item in items if item.get("type") == "dataforseo_trends_graph"),
        None,
    )
    if not graph:
        raise ValueError("DataForSEO Trendsの時系列データが返されませんでした。")
    graph_keywords = graph.get("keywords") or keywords
    series = [{"keyword": keyword, "points": []} for keyword in keywords]
    keyword_indexes = {
        str(keyword): index for index, keyword in enumerate(graph_keywords)
    }
    for item in graph.get("data") or []:
        values = item.get("values") or []
        for output in series:
            value_index = keyword_indexes.get(output["keyword"])
            ...
            output["points"].append(
                {
                    "date": str(item.get("date_from", ""))[:10],
                    "dateTo": str(item.get("date_to", ""))[:10],
                    "value": float(value),
                }
            )
    if not any(output["points"] for output in series):
        raise ValueError("DataForSEO Trendsの系列がすべて空でした。")
    cost_usd = float(task.get("cost") or payload.get("cost") or 0)
    with database_connection() as connection:
        connection.execute(
            """
            INSERT INTO dataforseo_trends_cache(
                keywords_key, start_date, end_date, location_name,
                series_json, cost_usd, fetched_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(keywords_key, start_date, end_date, location_name) DO UPDATE SET
...
def normalize_chart_month_range(
    start_month: str | None,
    end_month: str | None,
) -> tuple[str, str, str, str]:
    start_month = str(start_month or "").strip()
    end_month = str(end_month or "").strip()
    month_pattern = re.compile(r"^\d{4}-(0[1-9]|1[0-2])$")
    if start_month and not month_pattern.fullmatch(start_month):
        raise ValueError("通常チャートの開始年月はYYYY-MM形式で指定してください。")
    if end_month and not month_pattern.fullmatch(end_month):
        raise ValueError("通常チャートの終了年月はYYYY-MM形式で指定してください。")
    if start_month and end_month and start_month > end_month:
        raise ValueError("通常チャートの開始年月は終了年月以前にしてください。")

    start_date = f"{start_month}-01" if start_month else ""
    if end_month:
        end_value = datetime.strptime(f"{end_month}-01", "%Y-%m-%d")
        if end_value.month == 12:
            next_month = end_value.replace(year=end_value.year + 1, month=1)
        else:
            next_month = end_value.replace(month=end_value.month + 1)
        end_exclusive = next_month.strftime("%Y-%m-%d")
    else:
        end_exclusive = ""
    return start_month, end_month, start_date, end_exclusive


def filter_chart_points_by_month(
    points: list[PricePoint],
    start_date: str,
    end_exclusive: str,
) -> list[PricePoint]:
    filtered = [
        point
        for point in points
        if (not start_date or point.date_text >= start_date)
        and (not end_exclusive or point.date_text < end_exclusive)
    ]
    if not filtered:
        raise ValueError("指定した年月内に通常チャートへ表示できる株価データがありません。")
    return filtered
...
def build_stock_payload(
    ...
    delayed_buy_days = normalize_delayed_buy_days(delayed_buy_days)
    drawdown_buy_percent = normalize_drawdown_buy_percent(drawdown_buy_percent)
    chart_start_month, chart_end_month, chart_start_date, chart_end_exclusive = (
        normalize_chart_month_range(chart_start_month, chart_end_month)
    )
    has_custom_chart_range = bool(chart_start_month or chart_end_month)
    try:
        display_result = replace(
            fetch_stock_data(symbol, "1mo" if has_custom_chart_range else period),
            security_type=security_type,
        )
    except Exception:
        cached_result = cached_stock_result(symbol)
    ...
    if has_custom_chart_range:
        display_result = replace(
            listing_result,
            points=filter_chart_points_by_month(
    ...
    payload["chartRange"] = {
        "custom": has_custom_chart_range,
        "startMonth": chart_start_month,
        "endMonth": chart_end_month,
        "effectiveStart": display_result.points[0].date_text,
        "effectiveEnd": display_result.points[-1].date_text,
    }
    return payload
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .app-version { color:#94a3b8; font-size:12px; font-weight:700; }
    ...
    .search-panel {
      display:grid; grid-template-columns:1.8fr 1.1fr 1fr .9fr .9fr 1fr 1.35fr auto;
    ...
    .event-controls, .trends-controls {
    ...
    .trends-controls input { width:min(340px,100%); }
    .trends-controls .note { margin-left:2px; }
    .dataforseo-settings {
      width:100%; border-top:1px solid #dbe4ef; padding-top:8px;
    }
    .dataforseo-settings summary {
      cursor:pointer; color:#475569; font-weight:700;
    }
    .dataforseo-settings-row {
      display:flex; align-items:end; flex-wrap:wrap; gap:8px; margin-top:10px;
    }
    .dataforseo-settings-row label { min-width:230px; flex:1; }
    .dataforseo-settings-row input { width:100%; }
    .top-run-status {
...
<body>
<main class="app">
  <div class="title-row"><h1>世界の株価・底練り判定</h1><span class="app-version">v__APP_VERSION__</span></div>
  ...
  <section class="panel search-panel">
    ...
    <div>
      <label for="chartStartMonth">開始年月</label>
      <input id="chartStartMonth" type="month" min="1900-01" title="入力時は通常チャート期間より優先します">
    </div>
    <div>
      <label for="chartEndMonth">終了年月</label>
      <input id="chartEndMonth" type="month" min="1900-01" title="指定月の末日まで表示します">
    </div>
    <div>
    ...
    <div class="note" style="grid-column:1/-1">開始年月・終了年月を入力した場合は、その範囲を通常チャートへ表示します。両方空欄なら期間プルダウンを使用します。</div>
  </section>
  ...
  <div class="trends-controls">
    <input id="trendsKeywords" autocomplete="off" placeholder="最大3語をカンマ区切り：任天堂 株価, 任天堂 暴落, 任天堂 買い時">
    <button id="fetchTrends" class="secondary" type="button" disabled>検索人気度を重ねる</button>
    <label class="simulation-toggle" title="各語を別々に取得し、低頻度語の0張り付きを抑えます。APIリクエストは検索語数分になります。">
      <input id="trendsAccurateMode" type="checkbox" checked>
      精度優先
    </label>
    <button id="trendScaleMode" class="secondary" type="button" disabled>各語を個別0〜100化</button>
    <button id="clearTrends" type="button" disabled>非表示</button>
    <span id="trendsStatus" class="note">精度優先：各語を単独取得します。同じ語の保存結果は再利用します。</span>
    <details class="dataforseo-settings">
      <summary>DataForSEO API設定</summary>
      <div class="dataforseo-settings-row">
        <label>API login
          <input id="dataforseoLogin" autocomplete="username" placeholder="API Access画面のlogin">
        </label>
        <label>API password
          <input id="dataforseoPassword" type="password" autocomplete="current-password" placeholder="API Access画面のpassword">
        </label>
        <button id="saveDataforseoCredentials" class="secondary" type="button">暗号化して保存</button>
        <button id="deleteDataforseoCredentials" type="button">保存済みを削除</button>
      </div>
      <div id="dataforseoCredentialStatus" class="note">認証状態を確認しています…</div>
    </details>
  </div>
...
<script>
  ...
  let stockData = null;
  let trendsData = null;
  let trendsScaleMode = "independent";
  ...
  function searchEstimateText() {
    ...
    const hasCustomRange = Boolean(
      ($("analysisStart")?.value || "").trim() ||
      ($("analysisEnd")?.value || "").trim() ||
      ($("chartStartMonth")?.value || "").trim() ||
      ($("chartEndMonth")?.value || "").trim()
    );
  ...
  function enteredTrendsKeywords() {
    return $("trendsKeywords").value
      .split(/[,、\n]+/)
      .map(value => value.trim())
      .filter((value,index,array) => value && array.indexOf(value) === index);
  }

  async function fetchTrendsForCurrentStock() {
    if (!stockData?.points?.length) {
      $("trendsStatus").textContent = "先に銘柄を検索してください。";
      return;
    }
    const keywords = enteredTrendsKeywords();
    if (!keywords.length || keywords.some(keyword => keyword.length < 2)) {
      $("trendsStatus").textContent = "2文字以上の検索語を1〜3語入力してください。";
      $("trendsKeywords").focus();
      return;
    }
    if (keywords.length > 3) {
      $("trendsStatus").textContent = "1タスクに指定できる検索語は3語までです。";
      $("trendsKeywords").focus();
      return;
    }
    const start = stockData.points[0].date;
    const end = stockData.points[stockData.points.length - 1].date;
    const accurateMode = $("trendsAccurateMode").checked;
    $("fetchTrends").disabled = true;
    $("trendsStatus").textContent = accurateMode
      ? `精度優先で各語を単独取得しています（最大${keywords.length}リクエスト）：${keywords.join(" / ")}`
      : `料金優先で${keywords.length}語を一括取得しています：${keywords.join(" / ")}`;
    try {
      const query = new URLSearchParams({
        start,
        end,
        locationName:"Japan",
        normalizationMode:accurateMode ? "independent" : "shared"
      });
      keywords.forEach(keyword => query.append("keyword",keyword));
      const response = await fetch("/api/dataforseo-trends?" + query.toString());
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "DataForSEO Trendsを取得できませんでした。");
      trendsData = body;
      trendsScaleMode = body.normalizationMode === "independent" ? "independent" : "shared";
      $("clearTrends").disabled = false;
      updateTrendsScaleControl();
      const pointCount = Math.max(0,...(body.series || []).map(item => (item.points || []).length));
      $("trendsStatus").textContent =
        `${body.normalizationMode === "independent" ? "精度優先・単独取得" : "料金優先・一括取得"}` +
        ` ${(body.keywords || []).length}語・最大${pointCount}点を表示` +
        (body.cacheUsed ? "（保存データ利用）" : "（新規取得）") +
        `・API送信${Number(body.requestCount || 0)}回` +
        (Number(body.cacheHitCount || 0) ? `・キャッシュ${Number(body.cacheHitCount)}語` : "") +
        (!body.cacheUsed ? `・API費用 $${Number(body.costUsd || 0).toFixed(4)}` : "") +
        (trendsScaleMode === "independent"
          ? "。各語の期間内ピークを個別に100へ換算しています。"
          : trendsScaleMode === "emphasized"
          ? "。0を欠損扱いし、観測された正の値だけを強調しています。"
          : "。3語共通の0〜100相対人気度です。");
      drawPriceChart();
    } catch (error) {
      $("trendsStatus").textContent = error.message;
    } finally {
      $("fetchTrends").disabled = !stockData?.points?.length;
    }
  }

  function clearTrendsOverlay() {
    trendsData = null;
    trendsScaleMode = $("trendsAccurateMode").checked ? "independent" : "shared";
    $("clearTrends").disabled = true;
    updateTrendsScaleControl();
    $("trendsStatus").textContent =
      "非表示にしました。検索語を変更して再取得できます。";
    drawPriceChart();
  }

  function trendSeriesPeak(seriesItem) {
    return Math.max(0,...(seriesItem?.points || [])
      .map(point => Number(point.value))
      .filter(Number.isFinite));
  }

  function trendSeriesPositiveRange(seriesItem) {
  ...
  function displayedTrendValue(seriesItem, rawValue) {
    const value = Math.max(0,Number(rawValue) || 0);
    if (trendsScaleMode === "shared") return Math.min(100,value);
    if (trendsScaleMode === "emphasized") {
      if (value <= 0) return null;
      const range = trendSeriesPositiveRange(seriesItem);
      if (!range || range.max <= range.min) return 100;
      return 10 + (value - range.min) * 90 / (range.max - range.min);
    }
    const peak = trendSeriesPeak(seriesItem);
    return peak > 0 ? Math.min(100,value * 100 / peak) : 0;
  }

  function updateTrendsScaleControl() {
    const button = $("trendScaleMode");
    const hasSeries = Boolean(trendsData?.series?.some(item => item.points?.length));
    button.disabled = !hasSeries;
    const independentlyFetched = trendsData?.normalizationMode === "independent";
    button.textContent = independentlyFetched
      ? trendsScaleMode === "emphasized"
        ? "単独取得スケールに戻す"
        : "0を欠損扱い・低頻度強調"
      : trendsScaleMode === "shared"
      ? "各語を個別0〜100化"
      : trendsScaleMode === "independent"
      ? "0を欠損扱い・低頻度強調"
      : "3語共通スケールに戻す";
  ...
  function toggleTrendsScaleMode() {
    if (!trendsData?.series?.length) return;
    const independentlyFetched = trendsData.normalizationMode === "independent";
    trendsScaleMode = independentlyFetched
      ? trendsScaleMode === "emphasized" ? "independent" : "emphasized"
      : trendsScaleMode === "shared"
      ? "independent"
      : trendsScaleMode === "independent"
      ? "emphasized"
      : "shared";
    updateTrendsScaleControl();
    $("trendsStatus").textContent = trendsScaleMode === "shared"
      ? "共通スケール表示：3語を同じ0〜100軸で比較しています。"
      : trendsScaleMode === "independent"
      ? independentlyFetched
        ? "精度優先表示：各検索語をAPIから単独取得した0〜100値です。"
        : "個別スケール表示：一括取得値の期間内ピークをそれぞれ100として換算しています。"
      : "低頻度強調表示：0はデータ不足として描画せず、観測された正の値だけを10〜100へ拡大しています。";
    drawPriceChart();
  }

  async function refreshDataforseoCredentialStatus() {
    try {
      const response = await fetch("/api/dataforseo/credentials");
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "認証状態を確認できませんでした。");
      $("dataforseoCredentialStatus").textContent = body.configured
        ? `設定済み（${body.maskedLogin}・${body.source === "environment" ? "環境変数" : "Windows暗号化保存"}）`
        : "未設定です。API Access画面のloginとpasswordを入力してください。";
    } catch (error) {
      $("dataforseoCredentialStatus").textContent = error.message;
    }
  }

  async function saveDataforseoCredentialSettings() {
    const login = $("dataforseoLogin").value.trim();
    const password = $("dataforseoPassword").value;
    try {
      const response = await fetch("/api/dataforseo/credentials", {
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({action:"save",login,password})
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "認証情報を保存できませんでした。");
      $("dataforseoPassword").value = "";
      await refreshDataforseoCredentialStatus();
    } catch (error) {
      $("dataforseoCredentialStatus").textContent = error.message;
    }
  }

  async function deleteDataforseoCredentialSettings() {
    try {
      const response = await fetch("/api/dataforseo/credentials", {
        method:"POST",
        headers:{"Content-Type":"application/json"},
      ...
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "認証情報を削除できませんでした。");
      $("dataforseoLogin").value = "";
      $("dataforseoPassword").value = "";
      await refreshDataforseoCredentialStatus();
    } catch (error) {
      $("dataforseoCredentialStatus").textContent = error.message;
  ...
  async function searchStock() {
    ...
    const query = new URLSearchParams({
      ...
      market:$("market").value,
      period:$("period").value,
      chartStartMonth:$("chartStartMonth").value,
    ...
    try {
      ...
      trendsData = null;
      $("fetchImportantEvents").disabled = false;
      $("fetchTrends").disabled = false;
      $("clearTrends").disabled = true;
      updateTrendsScaleControl();
      $("trendsStatus").textContent =
        "最大3語を入力すると、DataForSEO人気度を株価チャートへ重ねられます。";
  ...
  function nearestIndexByDate(data, dateText) {
    if (!data?.length || !dateText) return null;
    const target = Date.parse(dateText + "T00:00:00Z");
    if (!Number.isFinite(target)) return null;
    let low = 0, high = data.length - 1;
    while (low < high) {
      const mid = Math.floor((low + high) / 2);
      const midTime = Date.parse(data[mid].date + "T00:00:00Z");
      if (midTime < target) low = mid + 1;
      else high = mid;
    }
    if (low === 0) return 0;
    const current = Date.parse(data[low].date + "T00:00:00Z");
    const previous = Date.parse(data[low - 1].date + "T00:00:00Z");
    return Math.abs(current - target) < Math.abs(previous - target) ? low : low - 1;
  }

  function trendValuesNearDate(dateText) {
    const target = Date.parse(dateText + "T00:00:00Z");
    return (trendsData?.series || []).map(item => {
      let best = null;
      ...
      return bestDistance <= 35 * 86400000 && best
        ? {
            keyword:item.keyword,
            value:displayedTrendValue(item,best.value),
            rawValue:Number(best.value),
            scaleMode:trendsScaleMode
          }
        : null;
    }).filter(item => item && item.value != null);
  }

  function drawTrendsOverlay(ctx, data, bounds, x) {
    const series = trendsData?.series || [];
    if (!series.length) return;
    const colors = ["#ea580c","#7c3aed","#059669"];
    const trendY = value => bounds.bottom - Math.max(0,Math.min(100,Number(value))) *
      (bounds.bottom - bounds.top) / 100;
    series.forEach((item,seriesIndex) => {
      const mapped = (item.points || [])
        .map(point => ({
          ...point,
          index:nearestIndexByDate(data,point.date),
          displayValue:displayedTrendValue(item,point.value)
        }))
        .filter(point => point.index != null && Number.isFinite(Number(point.value)));
      ...
      mapped.forEach(point => {
        ...
        if (penDown) ctx.lineTo(x(point.index),trendY(point.displayValue));
        else ctx.moveTo(x(point.index),trendY(point.displayValue));
        penDown = true;
      });
      ...
      if (trendsScaleMode === "emphasized") {
        ...
        mapped.filter(point => point.displayValue != null).forEach(point => {
          ctx.beginPath();
          ctx.arc(x(point.index),trendY(point.displayValue),2.6,0,Math.PI * 2);
          ctx.fill();
        });
      }
    });
    ...
    [0,20,40,60,80,100].forEach(value => {
      const py = trendY(value);
      ctx.fillStyle = "#64748b";
      ctx.fillText(String(value),bounds.right + 8,py);
    });
    series.forEach((item,index) => {
      const legendY = bounds.top + 10 + index * 17;
      ctx.strokeStyle = colors[index % colors.length];
      ctx.lineWidth = 2;
      ctx.setLineDash([7,4]);
      ctx.beginPath();
      ctx.moveTo(bounds.right - 210,legendY);
      ctx.lineTo(bounds.right - 186,legendY);
      ctx.stroke();
      ctx.setLineDash([]);
      ctx.fillStyle = colors[index % colors.length];
      const suffix = trendsScaleMode === "independent"
        ? "（個別）"
        : trendsScaleMode === "emphasized"
        ? "（低頻度）"
        : "";
      ctx.fillText(item.keyword + suffix,bounds.right - 180,legendY);
    });
  }
  ...
  function drawPriceChart() {
    ...
    const bounds = {left:82,top:22,right:width-(trendsData ? 62 : 20),bottom:height-44};
    ...
    drawTrendsOverlay(ctx,data,bounds,x);
    chartViews.price = {canvas,data,bounds,min,max,currency:stockData.currency,events:stockData.importantEvents || [],trendsData};
  }
  ...
  function handlePriceHover(event) {
    ...
    const trendHtml = trendValuesNearDate(item.date).map(trend =>
      `<br>人気度（${escapeHtml(trend.keyword)}${trend.scaleMode === "independent" ? "・個別換算" : trend.scaleMode === "emphasized" ? "・低頻度強調" : ""}）：${Number(trend.value).toFixed(0)}`
    ).join("");
    showTooltip(event,
      `<strong>${item.date}</strong><br>` +
      `終値：${money(item.close, stockData.currency)}` + trendHtml
    );
  }
  ...
  $("fetchTrends").addEventListener("click",fetchTrendsForCurrentStock);
  $("trendScaleMode").addEventListener("click",toggleTrendsScaleMode);
  $("trendsAccurateMode").addEventListener("change",() => {
    if (trendsData) clearTrendsOverlay();
    $("trendsStatus").textContent = $("trendsAccurateMode").checked
      ? "精度優先：各語を単独取得します。同じ語の保存結果は再利用します。"
      : "料金優先：最大3語を一括取得します。低頻度語が0に丸められる場合があります。";
  });
  $("clearTrends").addEventListener("click",clearTrendsOverlay);
  $("trendsKeywords").addEventListener("keydown",event => {
    if (event.key === "Enter") fetchTrendsForCurrentStock();
  });
  ["chartStartMonth","chartEndMonth"].forEach(id => {
    $(id).addEventListener("change",() => {
      const custom = Boolean($("chartStartMonth").value || $("chartEndMonth").value);
      $("period").title = custom
        ? "開始年月・終了年月の指定が優先されています"
        : "";
    });
  });
  $("saveDataforseoCredentials").addEventListener("click",saveDataforseoCredentialSettings);
  $("deleteDataforseoCredentials").addEventListener("click",deleteDataforseoCredentialSettings);
  ...
  refreshCacheStatus();
  refreshDataforseoCredentialStatus();
  $("symbol").focus();
</script>
</body>
</html>
...
class StockAppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/stock":
            ...
            chart_start_month = query.get("chartStartMonth", [""])[0]
            chart_end_month = query.get("chartEndMonth", [""])[0]
        ...
        if parsed.path == "/api/dataforseo-trends":
            try:
                query = parse_qs(parsed.query)
                self.send_json(
                    dataforseo_trends_payload(
                        ...
                        query.get("start", [""])[0],
                        query.get("end", [""])[0],
                        query.get("locationName", ["Japan"])[0],
                        query.get("normalizationMode", ["independent"])[0],
                    )
                )
            except Exception as exc:
                self.send_json({"error": str(exc)}, 400)
        ...
        if parsed.path == "/api/dataforseo/credentials":
            try:
                self.send_json(dataforseo_credential_status())
            except Exception as exc:
                self.send_json({"error": str(exc)}, 400)
            return
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/dataforseo/credentials":
            try:
                content_length = min(
                    int(self.headers.get("Content-Length", "0")), 10000
                )
                body = json.loads(self.rfile.read(content_length) or b"{}")
                action = str(body.get("action", "save")).strip().lower()
                if action == "delete":
                    delete_dataforseo_credentials()
                elif action == "save":
                    ...
                    raise ValueError("認証情報の操作が正しくありません。")
                self.send_json(dataforseo_credential_status())
            except Exception as exc:
                self.send_json({"error": str(exc)}, 400)
            return
